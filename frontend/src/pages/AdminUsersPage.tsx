		...
		try {
			...
			if (!res.ok) {
				const body = await res
					.json()
					.catch(() => ({ message: "Échec de la mise à jour." }));
	...
	const handleDelete = async (user: AdminUser) => {
		...
		const confirmDelete = window.confirm(
			`Supprimer définitivement ${
				user.email ?? user.name ?? "cet utilisateur"
			} ?`
		);
		...
		try {
			...
			if (!res.ok) {
				const body = await res
					.json()
					.catch(() => ({ message: "Échec de la suppression." }));
					...
					<div className="flex items-center gap-3">
						<div className="text-sm text-slate-500 dark:text-slate-300">
							{query.isLoading ? (
								...
								<span>
									{displayTotal} utilisateur{displayTotal > 1 ? "s" : ""} · page{" "}
									{page} / {totalPages}
								</span>
							)}
						</div>
