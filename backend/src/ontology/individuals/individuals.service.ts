...
const resolveInitialClassId = (candidate?: string): string =>
    candidate && !GENERIC_CLASS_IRIS.has(candidate) ? candidate : candidate || OWL_THING;

const mergeClassId = (current?: string, candidate?: string): string => {
    if (candidate && (!current || GENERIC_CLASS_IRIS.has(current)) && !GENERIC_CLASS_IRIS.has(candidate)) {
        return candidate;
    }
...
export class IndividualsService extends OntologyBaseService {
    ...
    async getIndividualsForOntology(userIri: string, ontologyIri: string, preferredLang?: string): Promise<IndividualNode[]> {
        ...
        (data.results.bindings as Row[]).forEach((row) => {
            ...
            if (predicateState) predicateLabelStates.set(row.pred.value, predicateState);
            const predicateLabel = predicateLabelStates.get(row.pred.value)?.value || this.iriLocalName(row.pred.value);
            ...
            if (isUri) {
                ...
                const relatedLabel = nodeLabelStates.get(relatedId)?.value || this.iriLocalName(relatedId);
            ...
            const subjectBuckets = literalCandidates.get(id) ?? new Map<string, LiteralBucket>();
        ...
        for (const [subjectId, predicates] of literalCandidates.entries()) {
            ...
            for (const [predicateIri, info] of predicates.entries()) {
                ...
                entry.properties.push({
                    ...
                    predicateLabel: info.predicateLabel || this.iriLocalName(predicateIri),
        ...
        for (const node of map.values()) {
            node.label = nodeLabelStates.get(node.id)?.value || node.label || this.iriLocalName(node.id);
            ...
            if (node.properties.length === 1) {
                node.properties = node.properties.map((prop) => this.finalizeProperty(prop));
                continue;
            }
            ...
            node.properties = Array.from(uniq.values()).map((prop) => this.finalizeProperty(prop));
        }
    ...
    private finalizeProperty(prop: Property): Property {
        return {
            ...
            valueLabel: prop.valueLabel || (prop.isLiteral ? prop.value : this.iriLocalName(prop.value)),
        };
    }

    private computeLabelScore(candidateLang?: string, preferredLang?: string | null): number {
        if (!preferredLang) {
        ...
        if (normalizedCandidate.split('-')[0] === normalizedPreferred.split('-')[0]) return 4;
        return 2;
    }
    ...
    private pickLabel(
        ...
        if (!current || score > current.score || (score === current.score && lang && !current.lang)) {
    ...
    async getAllPersons(preferredLang?: string): Promise<IndividualNode[]> {
        ...
        (data.results.bindings as Row[]).forEach((row) => {
            ...
            if (row.grp?.value) {
                const groupState = this.pickLabel(groupLabelStates.get(row.grp.value), row.grpLabel?.value, row.grpLabelLang?.value, langPreference);
                if (groupState) {
                    groupLabelStates.set(row.grp.value, groupState);
                }
                const groupLabel = groupLabelStates.get(row.grp.value)?.value || this.iriLocalName(row.grp.value);
            ...
            if (!isLiteral) {
                ...
                valueLabel = valueLabelStates.get(row.val.value)?.value || this.iriLocalName(row.val.value);
            }
        ...
        for (const person of personMap.values()) {
            person.label = personLabelStates.get(person.id)?.value || person.label || this.iriLocalName(person.id);
            person.properties = person.properties.map((prop) => this.finalizeProperty(prop));
        }

        return Array.from(personMap.values());
    }

    async getPerson(personIri: string, preferredLang?: string): Promise<IndividualNode | null> {
        const langPreference = this.sanitizeLang(preferredLang);
        const data = await this.runSelect(`
            PREFIX rdf:   <http://www.w3.org/1999/02/22-rdf-syntax-ns#>
            PREFIX rdfs:  <http://www.w3.org/2000/01/rdf-schema#>
        ...
        rows.forEach((row) => {
            personLabelState = this.pickLabel(personLabelState, row.uLabel?.value, row.uLabelLang?.value, langPreference) ?? personLabelState;
        ...
        person.properties = person.properties.map((prop) => this.finalizeProperty(prop));
        person.label = personLabelState?.value || person.label || this.iriLocalName(personIri);
        return person;
    }
}
