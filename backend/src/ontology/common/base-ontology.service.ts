...
export abstract class OntologyBaseService {
	...
	private static readonly CACHE_TTL_MS = Number(
		process.env.ONTOLOGY_CACHE_TTL_MS ?? 10_000
	);
	...
	private readonly organizationOwnerCache = new Map<
		string,
		CacheEntry<boolean>
	>();
	...
	protected async isOrganizationOwner(
		userIri: string,
	...
	protected async commentExistsInGraph(
		iri: string,
		graphIri: string
	): Promise<boolean> {
	...
	protected buildLabelSelection(
		...
		if (preferredLang) {
			chunks.push(
				`OPTIONAL { ${resourceVar} rdfs:label ?${safeAlias}Preferred . FILTER(LANGMATCHES(LANG(?${safeAlias}Preferred), "${preferredLang}")) }`
			);
			coalesceOrder.push(`?${safeAlias}Preferred`);
		}
		chunks.push(
			`OPTIONAL { ${resourceVar} rdfs:label ?${safeAlias}NoLang . FILTER(LANG(?${safeAlias}NoLang) = "") }`
		);
	...
	protected async enforceWritePermission(
		userIri: string,
		ontologyIri: string
	): Promise<void> {
		...
		if (!isOwner) {
			throw new ForbiddenException(
				"Accès refusé. Vous n'avez pas les droits d'écriture sur cette ontologie."
			);
		}
	}
	...
	private setCachedValue<T>(
		...
		cache.set(key, {
			value,
			expiresAt: Date.now() + OntologyBaseService.CACHE_TTL_MS,
		});
	}

	private getCachedBoolean(
		cache: Map<string, CacheEntry<boolean>>,
		key: string
	): boolean | undefined {
	...
	private invalidateCompositeCache(
		cache: Map<string, CacheEntry<boolean>>,
		suffix?: string
	) {
		if (!suffix) {
