...
export class AuthService {
	...
	private async fetchLocalAccountByEmail(
		email: string
		...
		return {
			...
			roles: row.roles?.value
				? row.roles.value.split("||").filter(Boolean)
				: [],
		};
	}
	...
	async login(email: string, password: string) {
		...
		const token = jwt.sign(
			{ sub: row.userIri, email: this.normalizeEmail(email) },
			this.JWT_SECRET,
			{
				expiresIn: "12h",
			}
	...
	async adminListUsers(
		...
		const selectedRole =
			options.role && allowedRoles.has(options.role) ? options.role : undefined;
		...
		const countRes = await lastValueFrom(
			this.http.get(this.fusekiUrl, { params: countParams })
		);
	...
	async adminUpdateUser(
		...
		const exists = await this.askBoolean(
			`PREFIX core: <${CORE}> ASK { <${userIri}> a core:User }`
		);
		if (!exists) {
		...
		const escapedNormalizedEmail =
			typeof normalizedEmail === "string"
				? escapeSparqlLiteral(normalizedEmail)
				: null;
